  ...
  return (
    <Card className="mt-6">
      <div className="flex items-center gap-3 mb-4 p-6 pb-0">
        ...
        <Button title="Execute this action" size="sm" variant="primary" onClick={() => setShowCreate(!showCreate)}>
      ...
      {showCreate && (
        <div className="mx-6 p-4 rounded-lg border border-border/30 bg-surface/20 mb-4">
          ...
          <Button title="Execute this action" variant="primary" size="sm" onClick={handleCreate} loading={saving}>
            {t("createChain")}
          </Button>
        </div>
      )}
          ...
          <div className="flex flex-col gap-2">
            {chainEntries.map(([model, chain]) => (
              ...
              >
                ...
                <button aria-label="Action button"                   onClick={() => handleDelete(model)}
